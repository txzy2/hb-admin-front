...
const LoaderComponent: React.FC<LoaderProps> = props => {
  const [currentText, setCurrentText] = useState<string>(props.title.text || 'Загружаю...');
  const texts = ['Загружаю компоненты...', 'Оптимизирую стек...', 'Проверяю данные...']; // Массив текстов
  let index = 0;
  ...
  return (
    <div className={props.className || 'flex items-center gap-2'}>
      ...
      <div className='flex items-center gap-1'>
        {props.title.need && (
            <h3
              className={`text-[${props.title.size ? props.title.size : 16
                }px] `}
            >
              {currentText}
            </h3>
